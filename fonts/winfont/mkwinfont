...
def fon(name, fonts):
    ...
    # Compute length of resource table. 2 for the shift count, plus
    # 20 for each resource, plus 2 for end-of-table, plus 8 for
    # the "FONTDIR" resource name.
    # Resources are currently one FONTDIR plus n fonts. TODO: a 
    # VERSIONINFO resource would be nice too.
    resrcsize = 12 + 20 * (1 + len(fonts))
    ...
    # The font resources.
    for i in range(len(fonts)):
	restable = restable + word(0x8008) + word(1) + dword(0)
	restable = restable + word((q+len(resdata)) >> 4)
	start = len(resdata)
