/*
 * SHA-384 and SHA-512 algorithms as described at
 * 
 *   http://csrc.nist.gov/cryptval/shs.html
 */

/*
 * SHA-384 is a trivial hack on SHA-512: it uses a different
 * initialisation vector, and discards the last 64 bits of the
 * digest. In all other respects the algorithms are identical.
 */
...
#define ror(r,x,y) ( r = ((x) << (64-(y))) | (((x)) >> (y)) )
#define shr(r,x,y) ( r = (x) >> (y) )
...
#define ror(r,x,y) ( y < 32 ? rorL(r,x,y) : rorB(r,x,y) )
#define shrB(r,x,y) ( r.lo = x.hi >> ((y)-32), r.hi = 0 )
#define shrL(r,x,y) ( r.lo = (x.lo >> (y)) | (x.hi << (32-(y))), \
		      r.hi = x.hi >> (y) )
#define shr(r,x,y) ( y < 32 ? shrL(r,x,y) : shrB(r,x,y) )
...
#define Maj(r,t,x,y,z) ( and(r,x,y), and(t,x,z), xor(r,r,t), \
			 and(t,y,z), xor(r,r,t) )
#define bigsigma0(r,t,x) ( ror(r,x,28), ror(t,x,34), xor(r,r,t), \
			   ror(t,x,39), xor(r,r,t) )
#define bigsigma1(r,t,x) ( ror(r,x,14), ror(t,x,18), xor(r,r,t), \
			   ror(t,x,41), xor(r,r,t) )
#define smallsigma0(r,t,x) ( ror(r,x,1), ror(t,x,8), xor(r,r,t), \
			     shr(t,x,7), xor(r,r,t) )
#define smallsigma1(r,t,x) ( ror(r,x,19), ror(t,x,61), xor(r,r,t), \
			     shr(t,x,6), xor(r,r,t) )
...
void SHA512_Core_Init(SHA512_Core_State *s) {
...
void SHA384_Core_Init(SHA512_Core_State *s) {
    static const uint64 iv[] = {
...
void SHA512_Block(SHA512_Core_State *s, uint64 *block) {
    uint64 w[80];
...
void SHA384_Bytes(SHA384_State *s, const void *p, int len) {
    SHA512_Bytes(s, p, len);
}
    
void SHA512_Final(SHA512_State *s, void *output) {
    int i;
    int pad;
...
void SHA384_Final(SHA384_State *s, void *output) {
    unsigned char digest[64];
    SHA512_Final(s, digest);
    memcpy(output, digest, 48);
}
...
int main(void) {
	...
	SHA384_Simple(tests[i].teststring,
		      strlen(tests[i].teststring), digest);
	for (j = 0; j < 48; j++) {
	    if (digest[j] != tests[i].digest384[j]) {
		fprintf(stderr,
			"\"%s\" digest384 byte %d should be 0x%02x, is 0x%02x\n",
			tests[i].teststring, j, tests[i].digest384[j],
			digest[j]);
		errors++;
	    }
	}
