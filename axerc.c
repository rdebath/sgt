...
#define RCNAME ".axerc"
#elif defined(MSDOS)
#define RCNAME "axe.rc"
#endif

static char *default_rc[] = {
    "# Default "RCNAME" generated by `axe -D'.",
    "#",
...
#elif defined(MSDOS)
    /*
     * Use environment variable AXERC if set. Otherwise, look for
     * AXE.RC in the same directory as AXE.EXE, if _that_ exists,
     * and failing everything else, try C:\AXE\AXE.RC.
     */
    if (getenv("AXERC"))
	strcpy (rcname, getenv("AXERC"));
    else {
	if ( (q = strrchr(pname, '\\')) != NULL) {
	    FILE *tempfp;

	    strncpy (rcname, pname, q+1-pname);
	    strcpy (rcname+(q+1-pname), "AXE.RC");
	    if ( (tempfp = fopen(rcname, "r")) != NULL)
		fclose (tempfp);
	    else
		strcpy (rcname, "C:\\AXE\\AXE.RC");
	} else
	    strcpy (rcname, "C:\\AXE\\AXE.RC");
    }
#endif
