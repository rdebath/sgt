 ...
 * of a filename / hostname / email address / that general order of
 * magnitude, which isn't all that much).
 * 
...
static int wc_match_fragment(const char **fragment, const char **target)
{
	...
	if (*f == '\\') {
	    ...
	    if (CASEFOLD((unsigned char)f[1]) != CASEFOLD((unsigned char)*t))
		...
		if (f[1] == '-') {
		    ...
		    /*
		     * Matching character classes case-
		     * insensitively is fraught with weirdness;
		     * what do you do with [Z-a]? Uppercase or
		     * lowercase the endpoints and then swap them
		     * to give [A-Z]/[a-z]?
		     * 
		     * My choice is to leave the class definition
		     * itself completely case-sensitive, but to
		     * match both upper- and lowercase forms of the
		     * input character against it. So [Z-a] really
		     * does mean Z[\]^_`a, but z and A also match
		     * as well. In other words, my case-insensitive
		     * matching is defined as `is there any case-
		     * transformed variant of the input string
		     * which would match the given case-sensitive
		     * wildcard?'.
		     */
#ifdef CASE_INSENSITIVE
		    ourchrU = toupper(ourchr);
		    ourchrL = tolower(ourchr);
		    if (ourchrU >= lower && ourchrU <= upper)
			matched = 1;
		    if (ourchrL >= lower && ourchrL <= upper)
			matched = 1;
#endif /* CASE_INSENSITIVE */

		} else {
		    matched |= (CASEFOLD((unsigned char)*t) ==
				CASEFOLD((unsigned char)*f));
		    f++;
		}
	    }
	    if (invert == matched)
	     ...
	     */
	    if (CASEFOLD((unsigned char)*f) != CASEFOLD((unsigned char)*t))
		return 0;
	    f++;
	}
	/*
...
/*
 * Some of the test (wildcard,string) pairs will only match when
 * matching is case-insensitive, or only when it's case-sensitive.
 */
#ifdef CASE_INSENSITIVE
#define ONLY_IF_INSENSITIVE 1
#define ONLY_IF_SENSITIVE 0
#else
#define ONLY_IF_INSENSITIVE 0
#define ONLY_IF_SENSITIVE 1
#endif /* CASE_INSENSITIVE */
...
const struct test fragment_tests[] = {
    ...
    {__LINE__, "abc", "abcd", 1},
    {__LINE__, "abcd", "abc", 0},
    {__LINE__, "ab[cd]", "abc", 1},
    {__LINE__, "ab[cd]", "abd", 1},
    {__LINE__, "ab[cd]", "abe", 0},
    {__LINE__, "ab[^cd]", "abc", 0},
    {__LINE__, "ab[^cd]", "abd", 0},
    {__LINE__, "ab[^cd]", "abe", 1},
    {__LINE__, "ab\\", "abc", -WC_TRAILINGBACKSLASH},
    {__LINE__, "ab\\*", "ab*", 1},
    {__LINE__, "ab\\?", "ab*", 0},
    {__LINE__, "ab?", "abc", 1},
    {__LINE__, "ab?", "ab", 0},
    {__LINE__, "ab[", "abc", -WC_UNCLOSEDCLASS},
    {__LINE__, "ab[c-", "abb", -WC_UNCLOSEDCLASS},
    {__LINE__, "ab[c-]", "abb", -WC_INVALIDRANGE},
    {__LINE__, "ab[c-e]", "abb", 0},
    {__LINE__, "ab[c-e]", "abc", 1},
    {__LINE__, "ab[c-e]", "abd", 1},
    {__LINE__, "ab[c-e]", "abe", 1},
    {__LINE__, "ab[c-e]", "abf", 0},
    {__LINE__, "ab[e-c]", "abb", 0},
    {__LINE__, "ab[e-c]", "abc", 1},
    {__LINE__, "ab[e-c]", "abd", 1},
    {__LINE__, "ab[e-c]", "abe", 1},
    {__LINE__, "ab[e-c]", "abf", 0},
    {__LINE__, "ab[^c-e]", "abb", 1},
    {__LINE__, "ab[^c-e]", "abc", 0},
    {__LINE__, "ab[^c-e]", "abd", 0},
    {__LINE__, "ab[^c-e]", "abe", 0},
    {__LINE__, "ab[^c-e]", "abf", 1},
    {__LINE__, "ab[^e-c]", "abb", 1},
    {__LINE__, "ab[^e-c]", "abc", 0},
    {__LINE__, "ab[^e-c]", "abd", 0},
    {__LINE__, "ab[^e-c]", "abe", 0},
    {__LINE__, "ab[^e-c]", "abf", 1},
    {__LINE__, "ab[a^]", "aba", 1},
    {__LINE__, "ab[a^]", "ab^", 1},
    {__LINE__, "ab[a^]", "abb", 0},
    {__LINE__, "ab[^a^]", "aba", 0},
    ...
    {__LINE__, "ab[^\\[-\\]]", "ab]", 0},
    {__LINE__, "ab[^\\[-\\]]", "ab^", 1},
    {__LINE__, "ab[a-fA-F]", "aba", 1},
    {__LINE__, "ab[a-fA-F]", "abF", 1},
    {__LINE__, "ab[a-fA-F]", "abZ", 0},
    {__LINE__, "abc", "aBc", ONLY_IF_INSENSITIVE},
    {__LINE__, "aBc", "abc", ONLY_IF_INSENSITIVE},
    {__LINE__, "ab[cd]", "abC", ONLY_IF_INSENSITIVE},
    {__LINE__, "ab[cd]", "abD", ONLY_IF_INSENSITIVE},
    {__LINE__, "ab[cd]", "abE", 0},
    {__LINE__, "ab[CD]", "abc", ONLY_IF_INSENSITIVE},
    {__LINE__, "ab[CD]", "abd", ONLY_IF_INSENSITIVE},
    {__LINE__, "ab[CD]", "abe", 0},
    ...
    {__LINE__, "ab[CD]", "abD", 1},
    {__LINE__, "ab[CD]", "abE", 0},
    {__LINE__, "ab[^cd]", "abC", ONLY_IF_SENSITIVE},
    {__LINE__, "ab[^cd]", "abD", ONLY_IF_SENSITIVE},
    {__LINE__, "ab[^cd]", "abE", 1},
    {__LINE__, "ab[^CD]", "abc", ONLY_IF_SENSITIVE},
    {__LINE__, "ab[^CD]", "abd", ONLY_IF_SENSITIVE},
    {__LINE__, "ab[^CD]", "abe", 1},
    ...
    {__LINE__, "ab[^CD]", "abE", 1},
    {__LINE__, "ab[c-e]", "abB", 0},
    {__LINE__, "ab[c-e]", "abC", ONLY_IF_INSENSITIVE},
    {__LINE__, "ab[c-e]", "abD", ONLY_IF_INSENSITIVE},
    {__LINE__, "ab[c-e]", "abE", ONLY_IF_INSENSITIVE},
    {__LINE__, "ab[c-e]", "abF", 0},
    {__LINE__, "ab[C-E]", "abb", 0},
    {__LINE__, "ab[C-E]", "abc", ONLY_IF_INSENSITIVE},
    {__LINE__, "ab[C-E]", "abd", ONLY_IF_INSENSITIVE},
...
const struct test full_tests[] = {
    ...
    {__LINE__, "*a*", "park", 1},
    {__LINE__, "*a*", "perk", 0},
    {__LINE__, "?b*r?", "abracadabra", 1},
    {__LINE__, "?b*r?", "abracadabr", 0},
    {__LINE__, "?b*r?", "abracadabzr", 0},
};
...
int main(void)
{
	...
	if (aret != eret) {
	    printf("failed line %d: /%s/ against /%s/ returned %d not %d\n",
		   fragment_tests[i].line,
