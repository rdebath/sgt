...
int des_encipher(word32 *output, word32 L, word32 R, DESContext *sched) {
...
int des_decipher(word32 *output, word32 L, word32 R, DESContext *sched) {
    word32 swap, s0246, s1357;
...
void des_test(word32 key_msw, word32 key_lsw,
             word32 Lplain, word32 Rplain,
             word32 Lcipher, word32 Rcipher) {
    word32 out[2];
    DESContext sched;
    des_key_setup(key_msw, key_lsw, &sched);
    des_encipher(out, Lplain, Rplain, &sched);
    if (out[0] != Lcipher || out[1] != Rcipher) {
        printf("error: %08x %08x\n", out[0], out[1]);
    } else
        printf("ok\n");
    des_decipher(out, Lcipher, Rcipher, &sched);
    if (out[0] != Lplain || out[1] != Rplain) {
        printf("error: %08x %08x\n", out[0], out[1]);
    } else
        printf("ok\n");
}
...
int main(void) {
    des_test(0x12345678, 0x87654321,
             0xabcddcba, 0xfedccdef,
             0x784eb1b7, 0x6fd1a0fc);
    des_test(0x7a6b7d6c, 0x9e0d1e4c,
             0x53debb4b, 0x5fb921de,
             0x00000000, 0x00000000);
    des_test(0x7a6b7d6c, 0x9e0d1e4c,
             0xb3c6a9f5, 0xd6e1f2c8,
             0xa68687f8, 0x719f7478);
    return 0;
}
